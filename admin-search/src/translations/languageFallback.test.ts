...
// key ("@jhb.software/payload-admin-search:escapeHint") in the admin UI.
...
describe('admin languages without bundled plugin strings', () => {
  it('resolves a plugin key to the English string instead of the raw key', async () => {
    const config = adminSearchPlugin({})({
      collections: [],
    ...
    expect(i18n.t('@jhb.software/payload-admin-search:escapeHint' as never)).toBe('ESC')
  })
})
