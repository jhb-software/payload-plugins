...
async function findByIDCached({
  ...
  // and context.pagesPluginDraft is write-once (set only by the top-level operation) —
  // a draft and non-draft lookup for the same parent cannot collide in the same cache.
  ...
  if (!parentPromise) {
    parentPromise = req.payload
      .findByID({
        ...
        depth: 0,
        disableErrors: true,
        draft: req.context.pagesPluginDraft === true,
        locale,
        overrideAccess: true,
        req: {
          ...req,
