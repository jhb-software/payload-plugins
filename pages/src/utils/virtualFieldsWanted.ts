 ...
 *
 * `beforeRead` has no access to the operation's `select` (Payload does not pass it), so the
 * decision has to be made in `beforeOperation` and carried on `req.context`. A request outlives
