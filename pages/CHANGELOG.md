...
- fix: a `create` or `update` passing a `select` which asks for none of the virtual fields now computes `path` and `breadcrumbs` correctly. Previously the select stripped the fields the computation reads before the `afterChange` hooks ran, so downstream hooks received a document without a `path`. The fields needed for the computation are selected internally and stripped from the response again, which leaves the response exactly as the caller selected it.
