 ...
 * `req.context` so that the meta and point field hooks share a single API call.
 *
...
/**
 * beforeChange hook for the **location metadata JSON field**.
 *
 * When `{pointFieldName}_address` contains a string, geocodes it server-side
 * and returns the location metadata as the field value.
 */
export const createMetaBeforeChangeHook = (options: { pointFieldName: string }): FieldHook => {
