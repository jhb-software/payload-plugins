...
- add `createPayloadBudget` helper that returns a drop-in `{ budget, collection }` pair persisting usage to a Payload collection, with built-in `'daily'` / `'monthly'` period and `'user'` / `'global'` scope resolvers. Usage rows are keyed by `(scope, period, model)` so per-model spend is retained for cost attribution; the budget cap aggregates totals across every model in the bucket.
