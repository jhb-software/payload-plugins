...
| `access`          | `(req) => boolean \| Promise<boolean>`                              | No       | Gates every plugin surface (endpoints, admin view, nav link). Defaults to users of the admin user collection (`admin.user`) — see [Production considerations](#production-considerations)               |
...
- **Access defaults to any admin user.** Without a custom `access`, every user of the admin user collection (`admin.user`) can use the agent and forward CMS content to your LLM provider. Gate it to specific roles in real deployments. Conversations always belong to a `users` document, so users of other auth collections admitted via `access` can chat but see no saved conversations.
